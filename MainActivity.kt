...
class MainActivity : AppCompatActivity() {
    ...
    private val selectFolderLauncher = registerForActivityResult(
        ...
        if (result.resultCode == Activity.RESULT_OK) {
            ...
            if (treeUri != null) {
                ...
                statusTextView.text = "ההגדרה הושלמה בהצלחה! לחץ על הכפתור הירוק."
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedUri != null) {
            statusTextView.text = "מוכן לעבודה! חבר את ה-USB ולחץ על הכפתור."
        } else {
            statusTextView.text = "אנא בצע הגדרה ראשונית בעזרת הכפתור למטה."
        }
        ...
        btnTransfer.setOnClickListener {
            ...
            if (uri == null) {
                Toast.makeText(this, "יש לבצע הגדרה ראשונית תחילה", Toast.LENGTH_LONG).show()
            } else {
                startTransfer(uri)
            }
        }
    ...
    private fun startTransfer(treeUri: Uri) {
        statusTextView.text = "מעתיק תמונות... נא לא לנתק את המכשיר!"
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            withContext(Dispatchers.Main) {
                ...
                statusTextView.text = "ההעתקה הסתיימה! הועברו $counter תמונות."
                Toast.makeText(this@MainActivity, "הסתיים! הועברו $counter תמונות", Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun saveSourceUri(uri: Uri) {
        getPreferences(Context.MODE_PRIVATE).edit().putString("uri", uri.toString()).apply()
    }
